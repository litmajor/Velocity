...
- U-1: ~~House-edge distribution under volatility shaping unknown~~ — MEASURED (fairness-transparency phase, `npm run rtp:study`, 200k rounds/config, docs/FAIRNESS.md §5). Result: the nominal 1% edge does NOT hold. CALM ≈ 84% RTP (16% edge); CHAOS is house-LOSING (RTP up to ~209% at 3x+ cashouts) and is entered deterministically after 5 sub-1.5× crashes — an observable, exploitable +EV regime. Elasticity ≥ 2 and the RESET preset are also player-positive (RTP 1.5–1.7). This is now a confirmed economic defect (re-classified as finding E-1 below) rather than an unknown. **E-1 FIXED in the economic-hardening phase** — see §7 item 2 and docs/ECONOMICS.md.
...
- After the economic-hardening phase: 10 files, **130 tests, 130 passing** — added `economics.test.ts` (analytic RTP-band sweep of the whole multiplier range incl. hostile profiles, empirical-vs-theory RTP at 6 thresholds × 13 configurations with 4σ tolerances, distribution quantiles, 11 adaptive public-information strategies, cryptographic-fairness regressions).
...
2. ~~**Fairness transparency** (D-1..D-3, U-1)~~ — DONE: blinded commit/reveal of the full crash mapping + standalone verifier (docs/FAIRNESS.md); RTP study run. NEW BLOCKER from the study — **E-1 (economic, HIGH)**: volatility regimes break the house edge in both directions (CHAOS/RESET/elasticity≥2 are player-positive up to ~2× RTP and CHAOS entry is publicly predictable; CALM ≈ 16% effective edge vs the advertised 1%). The shaping model needs an economic redesign or hard RTP bounds before real money. **E-1 FIXED (economic-hardening phase)**: the multiplicative crash modifier and `r^volatility` warp were replaced by a bounded EdgeProfile inverse-CDF model — RTP(m) = (1−h)·φ(m) ∈ [0.9405, 0.99] for every multiplier and every regime/mix/elasticity/steering combination, clamped inside the pure derivation; scheduled tilt-low rounds removed as economically incoherent. Root cause, contract, and methodology in docs/ECONOMICS.md; regression suite `test/economics.test.ts`; study `npm run rtp:study`.
...
Economic-hardening phase (E-1):

- `src/core/volatility-engine/index.ts` — replaced multiplicative `adjustCrash` with the bounded EdgeProfile model: `deriveProfile` (pure, clamped resolution of regime/mix/elasticity/steering into `{beta, lambda}`), `crashFromRPure` (deterministic inverse-CDF bisection with re-clamped bounds), `theoreticalSurvival` (closed form for tests); removed tilt-low scheduling.
- `src/core/fairness-engine/index.ts` — `computeProof` no longer warps `r`; derives the crash via `crashFromRPure`; snapshot resolves the shaping `volatility` knob into the committed profile.
- `src/core/fairness-engine/verifier.ts` — `recomputeCrashPoint` reconstructs via `crashFromRPure` from published data only.
- `scripts/rtp-study.ts` — rewritten for the profile model; reports ±3σ and closed-form theoretical RTP per cell.
- `test/economics.test.ts` — NEW economic regression suite; `test/fairness.test.ts` — updated to the profile model (regime → different committed profiles, boundedness, hostile-snapshot clamping).
- `docs/ECONOMICS.md` — NEW: economic contract, base-distribution math, E-1 root-cause derivation, redesigned model, statistical methodology, remaining risks.
